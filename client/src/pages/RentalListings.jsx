...
const emptyForm = { item_name: '', description: '', category: '', rate_type: 'daily', rate_amount: '', status: 'available', images: [], existingImageUrls: [] }
...
export default function RentalListingsPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    const amount = Number(form.rate_amount)
    if (!name || !description || !form.category || !['hourly', 'daily'].includes(form.rate_type) || !Number.isFinite(amount) || amount <= 0) {
      setFormError('Add an item name, description, category, and a positive rental rate.')
      return
    }
    setSubmitting(true)
    setFormError('')
